...
// ---------------------------------------------------------------
// Traducción a p5.js del sketch de Processing "TP2.pde"
// Equivalencias usadas:
//   SinOsc (processing.sound)        -> p5.Oscillator('sine')
//   size()                           -> createCanvas()
//   pushMatrix()/popMatrix()         -> push()/pop()
//   PFont / createFont()             -> textFont() con fuente del sistema
// El resto de las funciones (map, constrain, dist, bezierPoint,
// circle, rect, beginShape/vertex/endShape, colorMode HSB, etc.)
// existen igual en p5.js, así que se tradujeron casi 1 a 1.
// ---------------------------------------------------------------
...
let ultimoTiempo = 0;   // variable para almacenar el tiempo del último paso
let speed;              // velocidad de reproducción del tiempo

let controller_radio; // radio de los círculos de control

let play = false;
let play_control_X, play_control_Y; // posición del botón de play
...
let rotating = false; // controla la rotación de la pantalla
...
function setup() {
  ...
  pA_x = width * 0.3; // posición inicial de la punta de curva A
  pA_y = height * 0.6;
  pB_x = width * 0.7; // posición inicial de la punta de curva B
  pB_y = height * 0.3;

  contr_A_x = pA_x + (pB_x - pA_x) / 2; // posición inicial X centrada del punto de control A
  contr_A_y = pB_y + (pA_y - pB_y) / 2; // posición inicial Y centrada del punto de control A
  contr_B_x = contr_A_x;
  contr_B_y = contr_A_y;

  controller_radio = width / 20; // radio de los círculos de control
  time_counter = 0;
  speed = 5;
  ultimoTiempo = 0;
...
function draw() {
  ...
  // Cálculo de los puntos Bezier del paso actual para obtener su grosor.
  ...
  for (let i = 0; i < width_steps; i += 1) {
    ...
    if (i % 2 === 0) {
      if (current_step === i) {
        fill(color_activo, 75, 75); // Rojo si es la banda activa dependiendo de la frecuencia
      } else {
        fill(0); // Negro base
      }
    } else {
...
function mouseDragged() {
  ...
  // Verificación de si el mouse está dentro del radio de los círculos de control
...
function mousePressed() {
  // p5.sound necesita un gesto del usuario para arrancar el audio en el navegador
  userStartAudio();
